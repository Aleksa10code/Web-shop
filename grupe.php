...
.telo img{width:364px; height:250px; display:block; object-fit:cover;}
...
/* paginacija */
.paginacija{height:40px; margin:20px 0 70px;}
...
.telo img, .telo header, .telo main, .telo footer{width:100%;}
...
<?php
$sql="SELECT * FROM namestaj WHERE brojkomada>0 AND grupa='$grupa'";
...
$sql="SELECT * FROM namestaj WHERE brojkomada>0 AND grupa='$grupa' ORDER BY id DESC LIMIT $odakle,$postrani";
$upit=mysqli_query($conn,$sql);
...
<footer><?=number_format($red["cena"], 2)?> <small style="font-family:geomanist;">RSD</small></footer>
</a>
...
{$width = 44*$brojstrana+2*44;} else {$width = 44*$brojstrana+4*44;}
?>
...
/* na prvu stranu */
if($strana!==1){$x=1;
echo '<a href="'.$_SERVER['PHP_SELF'].'?strana='.$x.'&grupa='.$grupa.'"><<</a>';}	
...
<?php } else {echo '<a href="'.$_SERVER['PHP_SELF'].'?strana='.$x.'&grupa='.$grupa.'">'.$x.'</a>';	
}}}
